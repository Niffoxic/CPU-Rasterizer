...
namespace fox
{
    ...
    {
        r = (std::uint8_t)((rgba >>  0) & 0xFFu);
        g = (std::uint8_t)((rgba >>  8) & 0xFFu);
        b = (std::uint8_t)((rgba >> 16) & 0xFFu);
        a = (std::uint8_t)((rgba >> 24) & 0xFFu);
    }

    [[nodiscard]] static inline std::uint8_t rgba8_r(std::uint32_t rgba) noexcept { return (std::uint8_t)((rgba >>  0) & 0xFFu); }
    [[nodiscard]] static inline std::uint8_t rgba8_g(std::uint32_t rgba) noexcept { return (std::uint8_t)((rgba >>  8) & 0xFFu); }
    [[nodiscard]] static inline std::uint8_t rgba8_b(std::uint32_t rgba) noexcept { return (std::uint8_t)((rgba >> 16) & 0xFFu); }
    [[nodiscard]] static inline std::uint8_t rgba8_a(std::uint32_t rgba) noexcept { return (std::uint8_t)((rgba >> 24) & 0xFFu); }
    ...
    public:
        ...
        cpu_frame begin_frame() noexcept;
        cpu_frame try_begin_frame() noexcept;
